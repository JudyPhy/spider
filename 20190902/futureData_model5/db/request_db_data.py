...
def RequestHistoryRaceCardRows(race_date):
...
def RequestHistoryRaceResultsRows(race_date):
    ...
    return history_race_results_rows
...
def RequestCourseStandardTimes():
    course_standard_times_rows = {}  # rows
    if singleton_Scrub_DB.table_exists(common.COURSE_STANDARD_TIMES_TABLE):
        singleton_Scrub_DB.cursor.execute('select * from {}'.format(common.COURSE_STANDARD_TIMES_TABLE))
        course_standard_times_rows = singleton_Scrub_DB.cursor.fetchall()
        singleton_Scrub_DB.connect.commit()
    return course_standard_times_rows
