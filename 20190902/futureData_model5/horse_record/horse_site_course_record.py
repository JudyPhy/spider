...
def __getSiteCourseRecord(horse_code, site, course, history_raceResults_rows):
...
def GetHorseSiteCourseRecord(future_raceCard_rows, history_raceResults_rows):
    site_course_record_dict = {}  # race_date_No & {horse_No & [No1, No2, No3, No4, All]}
    for race_date_No, dict in future_raceCard_rows.items():
        if race_date_No not in site_course_record_dict.keys():
            site_course_record_dict[race_date_No] = {}
        for horse_No, row in dict.items():
            ...
            site_course_record_dict[race_date_No][horse_No] = __getSiteCourseRecord(horse_code, site, course, history_raceResults_rows)
    return site_course_record_dict
