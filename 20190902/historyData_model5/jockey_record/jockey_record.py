...
def GetJockeyRecord(sort_race_date_rows, sort_history_raceResults_rows):
    jockey_record_dict = {}  # race_date & {jockey & [No1, No2, No3, No4, All]}
    temp_jockey_record = {}  # jockey & [No1, No2, No3, No4, All]
    for race_date, rows in sort_race_date_rows.items():
        ...
        for row in rows:
            ...
            if cur_jockey not in temp_jockey_record.keys():
                temp_jockey_record[cur_jockey] = [0, 0, 0, 0, 0]
            # before
            records = temp_jockey_record[cur_jockey]
            jockey_record_dict[race_date][cur_jockey] = [records[0], records[1], records[2], records[3], records[4]]
        # after
        for row in rows:
            race_No = row['race_No']
            ...
            if (race_date_No in sort_history_raceResults_rows.keys()) and (horse_code in sort_history_raceResults_rows[race_date_No].keys()):
                ...
                if cur_plc not in common.words:
                    ...
                    cur_jockey = array_jockey[0].strip()
                    temp_jockey_record[cur_jockey][4] += 1
                    if int(cur_plc) == 1:
                        temp_jockey_record[cur_jockey][0] += 1
                    elif int(cur_plc) == 2:
                        temp_jockey_record[cur_jockey][1] += 1
                    elif int(cur_plc) == 3:
                        temp_jockey_record[cur_jockey][2] += 1
                    elif int(cur_plc) == 4:
                        temp_jockey_record[cur_jockey][3] += 1
