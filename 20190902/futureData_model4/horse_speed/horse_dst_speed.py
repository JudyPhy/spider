...
def __getDistanceSpeeds(horse_code, distance, history_raceResults_rows):
    total_distance = 0
    total_time = 0
    highest_speed = 0
    lowest_speed = 9999
...
def GetHorseDstSpeed(future_raceCard_rows, history_raceResults_rows):
    dst_speed_dict = {}  # race_date_No & {horse_No & [highest_speed, lowest_speed, avr_speed]}
    for race_date_No, dict in future_raceCard_rows.items():
        ...
        for horse_No, row in dict.items():
            ...
            dst_speed_dict[race_date_No][horse_No] = __getDistanceSpeeds(horse_code, distance, history_raceResults_rows)
