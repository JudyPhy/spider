...
def getOddsWave(race_date, race_No, horse_No, win_odds, today_race_start_time, immd_rows):
    ...
    if race_date_No in today_race_start_time.keys():
        ...
        before20_time = today_race_start_time[race_date_No][2]
        if ((datetime.datetime.now() - before20_time).total_seconds() > 0) and ((datetime.datetime.now() - before10_time).total_seconds() <= 0):  # 赛前10-20分钟
            base_odds = __getWinOddsByTime(race_No, horse_No, before20_time, immd_rows)
        elif (datetime.datetime.now() - before10_time).total_seconds() > 0:
        ...
        # print(race_No, base_odds, win_odds)
        if base_odds != 0:
            return (win_odds - base_odds) / base_odds
    return 1


def getOddsDiff(race_date, race_No, horse_No, win_odds, today_race_start_time, immd_rows):
    race_date_No = race_date + common.toDoubleDigitStr(race_No)
    if race_date_No in today_race_start_time.keys():
        base_time = today_race_start_time[race_date_No][0]
        before10_time = today_race_start_time[race_date_No][1]
        ...
        if ((datetime.datetime.now() - before20_time).total_seconds() > 0) and ((datetime.datetime.now() - before10_time).total_seconds() <= 0):  # 赛前10-20分钟
            base_odds = __getWinOddsByTime(race_No, horse_No, before20_time, immd_rows)
        elif (datetime.datetime.now() - before10_time).total_seconds() > 0:  # 赛前10分钟内
            base_odds = __getWinOddsByTime(race_No, horse_No, before10_time, immd_rows)
        else:
            base_odds = __getWinOddsByTime(race_No, horse_No, base_time, immd_rows)
        if base_odds != 0:
            return win_odds - base_odds
    return 0
