...
from update_model5 import update_data_horse
from update_model5 import update_data_trainer
from update_model5 import update_data_jockey
from update_model5 import update_data_going
from update_model5 import update_data_draw
from update_model5 import update_data_win_odds
...
def __updateRelativeData(today_table_rows, race_results_rows, race_card_history_rows, immd_rows, today_race_start_time,
                         update_table, race_No, horse_No, update_keys, cur_row, prev_row):
    if 'horse_code' in update_keys:
        update_data_horse.updateData(race_results_rows, race_card_history_rows, cur_row, prev_row, update_table,
                                     race_No, horse_No)
    if 'trainer' in update_keys:
        update_data_trainer.updateData(immd_rows, race_results_rows, cur_row, update_table, race_No, horse_No)
    ...
    if 'going' in update_keys:
        update_data_going.updateData(race_results_rows, cur_row, prev_row, update_table, race_No, horse_No)
    ...
    if 'win_odds' in update_keys:
        update_data_win_odds.updateData(immd_rows, today_race_start_time, cur_row, update_table, race_No, horse_No)
    ...
    for race_No, rows in dict.items():
        for horse_No, prev_row in rows.items():
            ...
            if cur_row != None:
                ...
                if (cur_exit_race == 1) or ('SCR' in cur_row['win_odds']) or ('SCR' in cur_row['pla_odds']):
                    print('horse exit race:', race_No, ' horse_no=', horse_No)
                    __deleteFromDb(update_table, cur_race_date, race_No, horse_No)
                    update_data_trainer.updateSameTrainerCount(immd_rows, cur_row, update_table, race_No)
                    continue
                # win_odds
                ...
                if prev_trainer != cur_trainer:
                    ...
                    __updateDb(update_table, 'trainer', race_No, horse_No, cur_trainer)
                    update_keys.append('trainer')
                # jockey_code
                prev_jockey = prev_row['jockey']
                ...
                if prev_jockey != cur_jockey:
                    ...
                    __updateDb(update_table, 'jockey', race_No, horse_No, cur_jockey)
                ...
                if prev_draw != cur_draw:
                    ...
                    __updateDb(update_table, 'draw', race_No, horse_No, cur_draw)
                ...
                __updateRelativeData(rows, race_results_rows, race_card_history_rows, immd_rows, today_race_start_time,
                                     update_table, race_No, horse_No, update_keys, cur_row, prev_row)
    singleton_ResultsDb.connect.commit()
