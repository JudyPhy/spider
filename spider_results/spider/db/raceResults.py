...
def process_RaceResultsRankItem(item):
    ...
    try:
        singleton_ScrubDb.cursor.execute(
            """select * from {} where race_date=%s and race_id=%s and horse_No=%s""".format(tableName),
            (item['race_date'], item['race_id'], item['horse_No']))
