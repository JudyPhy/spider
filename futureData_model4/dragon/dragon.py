...
FUTURE_RECE_CARD_TABLE = 't_race_card_20190227'
...
def __searchAllRaceCardData(today_date):
    ...
    # today
    if singleton_Scrub_DB.table_exists(FUTURE_RECE_CARD_TABLE):
        singleton_Scrub_DB.cursor.execute('select * from {} where race_date=%s'.format(FUTURE_RECE_CARD_TABLE), today_date)
        rows = singleton_Scrub_DB.cursor.fetchall()
        singleton_Scrub_DB.connect.commit()
        for row in rows:
            draw = row['draw'].replace('\xa0', '')
        ...
        common.log('dragon: Table[' + FUTURE_RECE_CARD_TABLE + '] not exist.')
