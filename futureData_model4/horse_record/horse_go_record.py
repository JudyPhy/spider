...
def getHorseGoingRecord(raceCard_rows, plc_dict, going_dict):
    ...
    going_record_dict = {}  # race_date_No & {horse_code & [No1, No2, No3, No4, Fail]}
    temp_plc_record = {}  # horse_code & {going & [No1, No2, No3, No4, Fail]}
    ...
    for race_date_No in sort_date_No_list:
        ...
        for row in rows:
            ...
            # 赛前
            if (horse_code in temp_plc_record.keys()) and (going in temp_plc_record[horse_code].keys()):
                record = temp_plc_record[horse_code][going]
            ...
            if going not in temp_plc_record[horse_code].keys():
                temp_plc_record[horse_code][going] = [0, 0, 0, 0, 0]
            plc = __getPlc(race_date_No, horse_code, plc_dict)
            if plc not in common.words:
                if int(plc) == 1:
                    temp_plc_record[horse_code][going][0] += 1
                elif int(plc) == 2:
                    temp_plc_record[horse_code][going][1] += 1
                elif int(plc) == 3:
                    temp_plc_record[horse_code][going][2] += 1
                elif int(plc) == 4:
                    temp_plc_record[horse_code][going][3] += 1
                elif int(plc) > 4:
                    temp_plc_record[horse_code][going][4] += 1
