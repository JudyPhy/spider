...
def getHorseCourseSpeed(raceCard_rows, results_rows):
    ...
    course_speed_dict = {}  # race_date_No & {horse_code & [highest_speed, lowest_speed, avr_speed]}
    temp_speed = {}  # horse_code & {course & [highest_speed, lowest_speed, all_dis, all_time]}
    ...
    for race_date_No in sort_date_No_list:
        ...
        for row in rows:
            ...
            # 赛前
            if (horse_code in temp_speed.keys()) and (course in temp_speed[horse_code].keys()):
                speed = temp_speed[horse_code][course]
                course_speed_dict[race_date_No][horse_code] = [speed[0], speed[1], speed[2]/(speed[3] + 0.00001)]
            else:
                course_speed_dict[race_date_No][horse_code] = [0, 0, 0]
            ...
            if time == None:
                ...
                if course not in temp_speed[horse_code].keys():
                    temp_speed[horse_code][course] = [0, 9999, 0, 0]
                distance = int(row['distance'])
                curSpeed = distance/(time + 0.00001)
                if curSpeed > temp_speed[horse_code][course][0]:
                    temp_speed[horse_code][course][0] = curSpeed
                if curSpeed < temp_speed[horse_code][course][1]:
                    temp_speed[horse_code][course][1] = curSpeed
                temp_speed[horse_code][course][2] += distance
                temp_speed[horse_code][course][3] += time

            # if 'V082' == horse_code:
            #     print('\n', race_date_No, course, distance, curSpeed)
            #     print(temp_speed[horse_code])
            #     print(course_speed_dict[race_date_No][horse_code])
