...
# 将原数据转成目标数据结构
def __toTargetStruct(row, results_rows, win_odds_dict, plc_dict, going_dict, pedigree_dst_dict,
                     ...
                     jockey_record_dict, jockey_recent_dict, jockey_hot_dict, jockey_hot_before4_dict, jockey_trainer_record_dict):
    race_date = row['race_date']    # 20190202 str
    race_no = row['race_No']
    __race_date_No = race_date + common.toDoubleDigitStr(race_no)
    horse_no = row['horse_No']
    horse_code = row['horse_code'].strip()
    race_id = source_datas.getRaceId(race_date, race_no, horse_code, results_rows)
    pla_odds = 1
    win_odds = source_datas.getWinOdds(__race_date_No, horse_code, win_odds_dict)
    plc = source_datas.getPlc(__race_date_No, horse_code, plc_dict)
    ...
    go_label = source_datas.getGoing(__race_date_No, horse_code, going_dict)
    ...
    if ('\xa0' in declar_horse_wt) and (plc == -1):
        declar_horse_wt = 0
    else:
    ...
    odd_trend = source_datas.getHorseOddsTrend(__race_date_No, horse_code, horse_odds_trend_dict)
    odd_sectional_trend = source_datas.getHorseOddsSectionalTrend(__race_date_No, horse_code, horse_odds_sectional_trend_dict)
    ...
    jockey_trainer_record = source_datas.getJockeyTrainerRecord(race_date, jockey_code + '__' + trainer_code, jockey_trainer_record_dict)
    ...
    item = (race_date, race_id, race_no, horse_no, pla_odds, win_odds, plc, horse_code, trainer_code, jockey_code,
            ...
            jockey_trainer_record[0] + jockey_trainer_record[1] + jockey_trainer_record[2] + jockey_trainer_record[3], jockey_trainer_record[4],
...
def main():
    raceCard_rows = __searchAllRaceCardData()
    results_rows = __getAllResultsRows()    # race_date_No & {horse_code & row}
    if len(raceCard_rows) > 0:
        ...
        for row in raceCard_rows:
            # win_odds
            win_odds_dict = data_dict['win_odds']
            # plc
            plc_dict = data_dict['plc']
            # going
            going_dict = data_dict['going']
            # pedigree_dst
            ...
            # horse_odds_trend
            horse_odds_trend_dict = data_dict['horse_odds_trend']
            # horse_odds_sectional_trend
            horse_odds_sectional_trend_dict = data_dict['horse_odds_sectional_trend']
            # horse_rest
            ...
            jockey_trainer_record_dict = data_dict['jockey_trainer_record']
            # jockey_hot
            ...
            # 将数据组装成目标数据结构
            cur_row = __toTargetStruct(row, results_rows, win_odds_dict, plc_dict, going_dict, pedigree_dst_dict,
                                       horse_record_dict, horse_recent_record_dict, horse_site_record_dict, horse_go_record_dict,
                                       horse_course_record_dict, horse_dst_record_dict, horse_cls_record_dict, horse_draw_record_dict,
                                       horse_jockey_record_dict, horse_speed_dict, horse_recent_speed_dict, horse_site_speed_dict,
                                       horse_go_speed_dict, horse_course_speed_dict, horse_dst_speed_dict, horse_cls_speed_dict,
