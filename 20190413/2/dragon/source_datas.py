...
def getJockeyRecent(race_date, jockey, dict):
    if (race_date in dict.keys()) and (jockey in dict[race_date].keys()):
        return dict[race_date][jockey]
    # print("JockeyRecent can't find:", race_date_No, jockey)
    return [0, 0, 0, 0, 0]
...
def prepareDatas(raceCard_rows, results_rows):
    ...
    data_dict['horse_last_dst_time_prev'], data_dict['horse_last_dst_time_ave'], data_dict['horse_last_dst_time_min'] = horse_last_dst_time.getHorseLastDstTimePrev(raceCard_rows, going_dict, plc_dict)
