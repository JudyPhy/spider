...
def __searchAllRaceCardData():
    ...
    for year in range(2014, now.year + 1):
        tableName = RECE_CARD_TABLE.replace('{0}', str(year))
    ...
    for year in range(2014, now.year + 1):
        ...
        if singleton_Scrub_DB.table_exists(tableName):
            ...
            for row in rows:
                ...
                race_date_id = array_date[2] + array_date[1] + array_date[0] + common.toDoubleDigitStr(row['race_No'])
                if race_date_id not in all.keys():
                    all[race_date_id] = {}
                horse_code = row['horse_code'].strip()
                if horse_code not in all[race_date_id].keys():
                    all[race_date_id][horse_code] = row
        else:
...
def __toTargetStruct(row, results_rows, win_odds_dict, plc_dict, going_dict, pedigree_dst_dict,
                     ...
                     rtg_dict, horse_dct_trend_dict, horse_act_trend_dict, horse_odds_trend_dict,
                     horse_rest_dict, horse_new_dis_dict, draw_record_dict, trainer_record_dict,
                     jockey_record_dict, jockey_recent_dict):
